...
package nl.demonsters.debugger {
	...
	public class MonsterDebugger
	{
		...
		// Error colors
		public static const COLOR_NORMAL		:int = 0x111111;
		public static const COLOR_ERROR			:int = 0xFF0000;
		public static const COLOR_WARNING		:int = 0xFF3300;
		...
		protected const HIGHLIGHT_COLOR			:int = 0xFFFF00;
		...
		protected var monitorFrames:int;
		 ...
		 */
		public static function snapshot(target:DisplayObject, color:int = 0x111111):void
		{
		 ...
		 * @param color: The color in the interface
		 */
		protected function snapshotInternal(target:DisplayObject, color:int = 0x111111):void
		{
			if (isEnabled)
			{
		 ...
		 */
		public static function trace(target:Object, object:*, color:int = 0x111111, functions:Boolean = false, depth:int = 4):void
		{
		 ...
		 * @param depth: The maximum depth of the trace
		 */
		protected function traceInternal(target:Object, object:*, color:int = 0x111111, functions:Boolean = false, depth:int = 4):void
		{
			if (isEnabled)
			{
			...
			if (maxDepth == -1 || currentDepth <= maxDepth)
			{
				...
				try
				{
					...
					else if (type == TYPE_ARRAY || type == TYPE_VECTOR)
					{
						...
						xml += createNode("node", {
							icon:			ICON_VARIABLE,
							label:			"length" + " (" + TYPE_int + ") = " + object["length"],
							name:			"length",
							type:			TYPE_int, 
							value:			object["length"], 
						...
						for (i = 0; i < keys.length; i++)
						{
							...
							if (childType == TYPE_STRING || childType == TYPE_BOOLEAN || childType == TYPE_NUMBER || childType == TYPE_INT || childType == TYPE_int || childType == TYPE_FUNCTION)
							{
								isXML = false;
								isXMLString = new XML();
					...
					else if (type == TYPE_STRING || type == TYPE_BOOLEAN || type == TYPE_NUMBER || type == TYPE_INT || type == TYPE_int)
					{
						isXML = false;
						isXMLString = new XML();
		...
		protected function monitorHandler(event:TimerEvent):void
		{
			if (isEnabled)
			{
				...
				var fps:int = monitorFrames / delta * 1000; // Miliseconds to seconds
				monitorFrames = 0;
				monitorTime = now;
